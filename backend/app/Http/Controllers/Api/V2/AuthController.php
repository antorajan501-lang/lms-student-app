...
class AuthController extends Controller
{
    ...
    public function login(Request $request): object
    {
        ...
        $targetUser = User::where($fieldType, $request->email)->first();
        if ($targetUser && StudentVerificationService::isBlocked($targetUser) && Hash::check($request->password, $targetUser->password)) {
            return response()->json(StudentVerificationService::apiPayload($targetUser), 403);
        }
        ...
        if (Settings('email_verification') == 1 && $user->email_verified_at == false) {
            return response()->json([
                'success' => false,
        ...
        if (StudentVerificationService::isBlocked($user)) {
            auth()->logout();

            return response()->json(StudentVerificationService::apiPayload($user), 403);
        }
