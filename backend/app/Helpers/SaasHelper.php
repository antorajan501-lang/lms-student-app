...
if (!function_exists('SaasDomain')) {
    function SaasDomain()
    {
        ...
        if (file_exists($saas_module) || file_exists($saas_module2)) {
            ...
            $module_status_path = base_path('modules_statuses.json');
            $module_status = [];
            if (file_exists($module_status_path)) {
                $module_status = json_decode(file_get_contents($module_status_path), true) ?? [];
            }
...
if (!function_exists('SaasInstitute')) {
    function SaasInstitute()
    {
        ...
        if ($hasInstituteTable) {
            ...
            $institute = collect();
            $institute->name = 'RMUTTO LMS';
            $institute->description = 'RMUTTO LMS';
