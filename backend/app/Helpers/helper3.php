...
if (!function_exists('GettingError')) {
    function GettingError($message, $url = '', $ip = '', $agent = '', $return = false)
    {
        ...
        if (\config('app.debug')) {
            Log::debug('Application error captured by GettingError.', [
                'message' => $message,
                'url' => $url,
                'ip' => $ip,
                'agent' => $agent,
        ...
        if (Schema::hasTable('error_logs')) {
            ...
            Log::error('error_logs table not found. Captured error: ' . $message);
        }
        if ($return) {
            return false;
        } else {
    ...
    function GenerateHomeContent($domain)
    {
        ...
        if (Schema::hasColumn('home_contents', 'key')) {
            ...
            if (Schema::hasTable('languages') && Schema::hasColumn('languages', 'code')) {
                $locales = DB::table('languages')->where('status', 1)->pluck('code')->filter()->unique()->all();
            } else {
                $locales = [app()->getLocale()];
            }

            foreach ($locales as $locale) {
                Cache::forget('homeContents_' . $locale . $domain);
            }
        }
    }
}
...
if (!function_exists('getRawHomeContents')) {
