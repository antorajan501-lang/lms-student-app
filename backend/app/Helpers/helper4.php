...
if (!function_exists('scormPackageDirectories')) {
    function scormPackageDirectories($link): array
    {
        $path = parse_url((string)$link, PHP_URL_PATH) ?: (string)$link;
        $path = str_replace('\\', '/', $path);

        if (!preg_match('#(?:^|/)(?:public/)?uploads/scorm/([^/]+)#', $path, $matches)) {
            return [];
        }

        $folder = $matches[1] ?? '';
        if ($folder === '' || str_contains($folder, "\0") || $folder === '.' || $folder === '..') {
            return [];
        }

        return array_values(array_unique([
            public_path('uploads/scorm/' . $folder),
            base_path('uploads/scorm/' . $folder),
        ]));
    }
}
...
if (!function_exists('dynamicContentAppend')) {
    function dynamicContentAppend($content = null)
    {
        try {
            ...
            if ($nodes) {
                foreach ($nodes as $node) {
                    ...
                    if (response($data)->status() == 200) {
                        ...
                        return $content ?: '<div class="alert alert-warning">Page content unavailable.</div>';
                    }
                }
            }
            ...
            Log::error('Dynamic content append failed.', [
