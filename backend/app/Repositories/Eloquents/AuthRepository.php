...
class AuthRepository implements AuthRepositoryInterface
{
    ...
    public function signup(object $request): array
    {
        ...
        if ($user->role_id == 2) {
            ...
            $user->status = 1;
            StudentVerificationService::markPending($user);
            $user->save();
        }
        $data = [
            'id'    => $user->id,
    ...
    public function login(object $request): array
    {
        ...
        if (StudentVerificationService::isBlocked($user)) {
            throw new HttpResponseException(
                response()->json(StudentVerificationService::apiPayload($user), 403)
            );
        }
    ...
    private function attemptUserCheck($request)
    {
        ...
        if (auth()->user()->role_id == 3) {
            if (StudentVerificationService::isBlocked(auth()->user())) {
                $user = auth()->user();
                auth()->logout();

                $result['type'] = false;
                $result['message'] = StudentVerificationService::message($user);
                return $result;
            }
