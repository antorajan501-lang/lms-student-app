    ...
    public function languages(object $request): object
    {
        $data = Language::when($search = $request->search, function ($language) use ($search) {
            ...
            ->map(function ($language) {
                $flag = (string)($language->flag_icon ?? '');
                // if flag is a stored file path, return public URL; otherwise return raw value (emoji or code)
                if (!empty($flag) && !str_starts_with($flag, ['http', 'data:']) && \Illuminate\Support\Facades\Storage::disk('public')->exists($flag)) {
                    $flag_url = asset('storage/' . $flag);
                } else {
                    $flag_url = $flag;
                }
                return [
