...
class LessonRepository implements LessonRepositoryInterface
{
    ...
    public function addLesson(object $request): bool|object
    {
        ...
        if (isset($course) && isset($chapter)) {
            ...
            $lesson->lesson_type = 'recorded_video';
            $lesson->course_session_id = null;
            ...
            $lesson->save();
            $this->logScormLessonStorage($lesson, 'lesson_repository_add');
    ...
    public function updateLesson(object $request): object
    {
        ...
        if (isset($course) && isset($chapter)) {
            ...
            $lesson->is_lock = (int)$request->is_lock;
            $lesson->update();
            $this->logScormLessonStorage($lesson, 'lesson_repository_update');
    ...
    private function lessonFileDelete($lesson): bool
    {
        try {
            ...
            if ($host == "SCORM") {
                ...
                if (!empty($index)) {
                    foreach (scormPackageDirectories($index) as $delete_dir) {
    ...
    private function logScormLessonStorage(Lesson $lesson, string $context): void
    {
        if (!in_array($lesson->host, ['SCORM', 'SCORM-AwsS3'], true)) {
            return;
        }

        Log::info('SCORM lesson database values stored', [
            'context' => $context,
            'lesson_id' => $lesson->id,
            ...
            'scorm_version' => $lesson->scorm_version,
            'scorm_identifier' => $lesson->scorm_identifier,
            'local_file_exists' => $this->scormLocalFileExists($lesson->video_url),
        ]);
    }

    private function scormLocalFileExists(?string $link): ?bool
    {
