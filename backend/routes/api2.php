...
Route::middleware(array_merge(['auth:api'], $verified))->group(function () {
    ...
    // ─── Student App Routes ──────────────────────────────────────────────────
    Route::prefix('student')->controller(\App\Http\Controllers\Api\V2\StudentDashboardController::class)->group(function () {
        Route::get('dashboard', 'dashboard');
        Route::get('course-progress/{courseId}', 'courseProgress');
    });

    Route::prefix('wishlist')->controller(\App\Http\Controllers\Api\V2\StudentWishlistController::class)->group(function () {
        Route::get('/', 'index');
