...
class Course extends Model
{
    ...
    {
        return $this->hasMany(CourseLike::class, 'course_id', 'id');
    }

    public function quizzes()
    {
        return $this->hasMany(\Modules\Quiz\Entities\OnlineQuiz::class, 'course_id', 'id');
    }
    ...
    {
        return $this->hasMany(CourseContentItem::class, 'course_id', 'id');
    }
    ...
    {
        if (Auth::check() && Auth::user()->role_id == 3 && Auth::user()->rajamangala_verified == 1 && $this->rmut_student_fees == 1) {
            return 0;
        }
        ...
        if (Auth::check() && Auth::user()->role_id == 3 && isModuleActive('Subscription') && isSubscribe()) {
            ...
            if ($type == 2) {
                if ($this->attributes['subscription'] == 0) {
                    ...
                    if (count($plans) != 0) {
                        $check = OrgSubscriptionCourseList::whereIn('plan_id', $plans)->where('course_id', $this->attributes['id'])->first();
                        if ($check) {
                            return 0;
                        } else {
                            return $price;
    ...
    public function getDiscountPriceAttribute()
    {
        ...
        $price = $this->attributes['discount_price'] ?? null;
    ...
    public function getIsLoginUserLikedAttribute()
    {
        if (!Auth::check()) {
            return false;
        }
